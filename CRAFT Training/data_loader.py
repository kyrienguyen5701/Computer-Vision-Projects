...
def random_crop(imgs, img_size, character_bboxes):
    ...
    if len(character_bboxes) > 0:
        for bboxes in character_bboxes:
            word_bboxes.append([[bboxes[:, :, 0].min(), bboxes[:, :, 1].min()],
                [bboxes[:, :, 0].max(), bboxes[:, :, 1].max()]])
...
class craft_base_dataset(data.Dataset):
    ...
    def inference_pseudo_bboxes(self, net, img, word_bbox, word, viz=False):
        word_image, MM = self.crop_image_by_bbox(img, word_bbox)
        ...
        img_torch = torch.from_numpy(imgproc.normalizeMeanVariance(input))
        ...
        # affinity_scores = scores[0, :, :, 1].cpu().data.numpy()
        ...
        pseudo_bboxes = watershed(input, bgr_region_scores, False)

        _tmp = []
        for i in range(pseudo_bboxes.shape[0]):
            if np.mean(pseudo_bboxes[i].ravel()) > 2:
                _tmp.append(pseudo_bboxes[i])
            else:
                print("Filter bboxes", pseudo_bboxes[i])

        pseudo_bboxes = np.array(_tmp, np.float32)
        if pseudo_bboxes.shape[0] > 1:
            index = np.argsort(pseudo_bboxes[:, 0, 0])
            pseudo_bboxes = pseudo_bboxes[index]

        confidence = self.get_confidence(real_char_nums, len(pseudo_bboxes))
    ...
    def save(self, image_name, image, region_scores, affinity_scores, confidence_mask):
        ...
        out_path = os.path.join(os.path.join(os.path.dirname(__file__) + '/output'), "%s_input.jpg" % image_name)
        print(out_path)
        if not os.path.exists(os.path.dirname(out_path)):
            os.mkdir(os.path.dirname(out_path))
        cv2.imwrite(out_path, output)

    def save_image(self, image_name, image, bboxes, affinity_bboxes, region_scores, affinity_scores, confidence_mask):
        ...
        outpath = os.path.join(os.path.join(os.path.dirname(__file__) + '/output'), image_name)
    ...
    def pull_item(self, index):
        ...
        if len(character_bboxes) > 0:
            ...
            affinity_scores, affinity_bboxes = self.gaussianTransformer.generate_affinity(region_scores.shape,
                character_bboxes, words)
        if self.viz:
            self.save_image(self.get_image_name(index), image.copy(), character_bboxes, affinity_bboxes, region_scores,
        ...
        if self.viz:
            self.save(self.get_image_name(index), cvimage, region_scores, affinity_scores, confidence_mask)
...
class ICDAR2013(craft_base_dataset):
    def __init__(self, net, icdar2013_folder, target_size=768, viz=False, debug=False):
        ...
        self.net = net
        self.net.eval()
        self.img_folder = os.path.join(icdar2013_folder, 'train/img')
        self.gt_folder = os.path.join(icdar2013_folder, 'train/gt')
        ...
        if len(word_bboxes) > 0:
            for i in range(len(word_bboxes)):
                if words[i] == '###' or len(words[i].strip()) == 0:
                    ...
                    continue
                pseudo_bboxes, bbox_region_scores, confidence = self.inference_pseudo_bboxes(self.net, image,
                    word_bboxes[i], words[i], viz=self.viz)
                confidences.append(confidence)
                cv2.fillPoly(confidence_mask, [np.int32(word_bboxes[i])], (confidence))
                new_words.append(words[i])
                character_bboxes.append(pseudo_bboxes)
    ...
    def load_gt(self, gt_path):
        ...
        for line in lines:
            newline = line.rstrip('\n').lstrip('\ufeff').split(' ')
            original_box = list(map(int, newline[:4]))
            new_box = [
